...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/products/packages?${queryString}`, { 
      cache: "no-store" 
    });
    ...
    if (response.ok) {
      ...
      // If backend fails, return static data with pagination
      console.warn("Backend packages endpoint failed, using static data");
      
      const startIndex = (page - 1) * limit;
      const endIndex = startIndex + limit;
      const paginatedPackages = staticPackages.slice(startIndex, endIndex);
      ...
      return NextResponse.json({
        ...
        data: {
          ...
          pagination: {
            currentPage: page,
            totalPages: Math.ceil(staticPackages.length / limit),
            totalPackages: staticPackages.length,
            hasNext: endIndex < staticPackages.length,
            hasPrev: page > 1
          }
        }
      });
