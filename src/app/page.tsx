...
export default function HomePage() {
  ...
  interface Category {
    ...
    // Include top-level productCount for backward compatibility if needed, though statistics is preferred
    productCount?: number;
  ...
  React.useEffect(() => {
    fetch("/api/products?productType=package&limit=10")
      ...
      .then((data: any) => {
        const rawPackages = data.success && Array.isArray(data.data)
          ...
          : data.success && data.data?.products
            ...
            : data.success && data.data?.packages
              ? data.data.packages
              : [];
        // Transform packages to ensure image field is correctly populated
        ...
        console.log("[HomePage] Transformed Packages:", transformedPackages);
        setPackages(transformedPackages);
      })
      .catch((err) => {
        console.error("[HomePage] Failed to fetch packages:", err);
        setPackages([]);
      })
      ...
      >
                ...
                {heroBanners.slice(0, 3).map((banner, index) => (
                  ...
                  >
                    <Image
                      ...
                      unoptimized // Standardize unoptimized for backend-sourced images
                    />
                  </Link>
                ))}
              </div>
                ...
                categories.filter(c => c.showInHomepage !== false).map((category) => (
                  ...
                  >
                    <div className="flex h-full flex-col items-center justify-center p-6 text-center">
                      ...
                      <h3 className="mb-1 text-lg font-medium group-hover:text-primary transition-colors">
                        {category.name}
                      </h3>
                      <p className="text-sm text-muted-foreground">
                        {category.statistics?.productCount || category.productCount || 0} products
                      </p>
                    </div>
                  </Link>
                ))
                      ...
                      <CardContent className="pb-4">
                        ...
                        <div className="mb-3 flex flex-wrap gap-1">
                          ...
                          {pkg.packageItems?.length > 3 && (
                            <span
                              ...
                              `}
                            >
                              +{pkg.packageItems.length - 3} more
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="text-2xl font-bold text-foreground">
                                {Number(pkg.price).toFixed(2)} EGP
                              </span>
                            </div>
                            <div className="flex items-center gap-2">
                              {pkg.originalPrice && (
                                <span className="text-sm text-muted-foreground line-through">
                                  {Number(pkg.originalPrice).toFixed(2)} EGP
                                </span>
                              )}
                              {pkg.savings && (
                          ...
                          <div className="text-right text-sm text-muted-foreground">
                            {pkg.packageItems?.length || 0} items
                          </div>
                        </div>
                      </CardContent>
                    </Card>
        ...
        >
          ...
          >
            <div
              className={`
                relative overflow-hidden rounded-3xl bg-primary px-6 py-16
                text-center text-primary-foreground shadow-2xl
                sm:px-12
                md:py-20
              `}
            >
              <div
                className={`
                  absolute top-0 left-0 h-64 w-64 -translate-x-1/2
                  -translate-y-1/2 rounded-full bg-white/10 blur-3xl
                `}
              />
              <div
                className={`
                  absolute bottom-0 right-0 h-64 w-64 translate-x-1/2
                  translate-y-1/2 rounded-full bg-black/10 blur-3xl
                `}
              />
              <div className="relative z-10 flex flex-col items-center space-y-8">
                <div className="space-y-4">
                  ...
                  <p
                    className={`
                      mx-auto max-w-2xl text-lg text-primary-foreground/80
                      md:text-xl
                    `}
                  >
                    Join thousands of dental professionals who trust MK for their
                    instruments and equipment needs.
                  </p>
                </div>
                <div className="flex flex-col gap-4 sm:flex-row">
                  <Link href="/products" className="w-full sm:w-auto">
                    <Button
                      className={`
                        h-12 w-full bg-white px-8 text-primary hover:bg-white/90
                        sm:w-auto
                      `}
                      size="lg"
                    >
